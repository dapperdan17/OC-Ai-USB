...
namespace OpenCodeUsbCreator
{
    ...
    public class MainForm : Form
    {
        ...
        void Configure(string root)
        {
            ...
            // bin\check-updates.ps1 - the script "Check for Updates.bat" invokes.
            // Written inline here rather than shipped in the payload, so the button
            // always works. It only CHECKS the latest published OpenCode version and
            // reports it; it never runs 'opencode upgrade', which installs via
            // curl/npm/brew/etc into a host location and would defeat the entire point
            // of a self-contained stick. Updating means replacing bin\opencode.exe.
            try
            {
...
Write-Host ''
";
                File.WriteAllText(Path.Combine(binDir, "check-updates.ps1"), ps);
                Log("Created: bin\\check-updates.ps1", Color.Green);
            }
            catch (Exception ex) { Log("check-updates.ps1 error: " + ex.Message, Color.Red); }
